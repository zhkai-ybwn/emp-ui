...
function findName(data: any, path: string): null | string {
  ...
  for (const item of data) {
    ...
    if (item.children) {
      const result = findName(item.children, path);
      if (result) {
        return result;
      }
    }
    ...
    async fetchServerMenuConfig() {
      ...
      if (getToken()) {
        try {
          ...
          // const newRoute = flatten(data).find(
          //   (el) => el.name === 'HealthDiagnosis'
          // );
          const newRoute = findName(
            data,
            '/smart-maintenance/health-diagnosis'
          );
