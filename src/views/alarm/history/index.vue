<template>
  <div class="app-container">
    <Breadcrumb :items="['报警管理', 'menu.alarm.history.list']" />
    <!-- 头部区域 -->
    <div class="header">
      <!-- 搜索栏 -->
    ...
    <div class="contain">
      ...
      >
        <template #columns>
          <a-table-column title="报警时间" align="center" :width="200">
          ...
          <a-table-column title="恢复时间" align="center" :width="200">
            <template #cell="{ record }">
              <span style="margin-right: 5px; color: rgb(255 255 255 / 26%)">{{
                record.recoverTime ? record.recoverTime.split(' ')[0] : '-'
              }}</span>
              <span>{{
                record.recoverTime
                  ? record.recoverTime.split(' ')[1]
                  : record.recoverTime
              }}</span>
            </template>
          </a-table-column>
          <a-table-column title="报警等级" align="center">
            ...
            <!-- <template #cell="{ record }">{{ record.alarmLevel.mes }} </template> -->
          </a-table-column>
          <a-table-column
            title="报警对象"
          ...
          <a-table-column title="操作" align="center">
            <template #cell="{ record }">
              <a-button type="text" @click="showDetailModal(record.alarmInfoId)"
                >详情</a-button
              ></template
            >
          </a-table-column>
...
<script setup lang="ts">
  ...
  import HistoryDetailModal from '@/components/manage/common/HistoryDetailModal.vue';
  import useEchart from '@/hooks/useEchart';
  import { listHisAlarms, Param, RealAlarm, listAreaInfos } from '@/api/alarm';
  ...
  const fetchData = async () => {
    data.tableLoad = true;
    try {
      const { data: res } = await listHisAlarms(getParam());
