    ...
    <a-card class="general-card" :title="$t('menu.system.role.list')">
      ...
      >
        <template #columns>
          ...
          <a-table-column v-if="false" title="数据权限" :width="130">
            <template #cell="{ record }">
      ...
      >
        <a-card title="基础信息" :bordered="false">
          <a-descriptions :column="2" bordered size="large">
            ...
            <a-descriptions-item v-if="false" label="数据权限">
              <a-skeleton v-if="detailLoading" :animation="true">
                <a-skeleton-line :rows="1" />
              </a-skeleton>
              <span v-else>
...
<script lang="ts" setup>
  ...
  const getMenuTree = () => {
    ...
    listMenuTree({ sort: ['parentId,asc', 'sort,asc', 'createTime,desc'] })
      .then((res) => {
