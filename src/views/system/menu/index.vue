    ...
    <a-card class="general-card" :title="$t('menu.system.menu.list')">
      ...
      >
        <template #columns>
          ...
          <a-table-column title="状态" align="center">
            <template #cell="{ record }">
              <a-switch
                ...
                :checked-value="1"
                :unchecked-value="2"
                :disabled="!checkPermission(['system:menu:update']) && hasRole"
          ...
          <a-table-column
            v-if="
              checkPermission(['system:menu:update', 'system:menu:delete']) &&
              hasRole
            "
            title="操作"
            align="center"
      ...
      >
        ...
        >
          ...
          <!-- <a-form-item v-if="form.type !== 3" label="是否内嵌" field="isIframe">
            <a-radio-group v-model="form.isIframe" type="button">
              <a-radio :value="true">是</a-radio>
              <a-radio :value="false">否</a-radio>
            </a-radio-group>
          </a-form-item> -->
...
<script lang="ts" setup>
  ...
  const clearComInfo = (v: any) => {
    if (form.value.type === 2 && v === true) {
    ...
    proxy.$refs.formRef.validate((valid: any) => {
      if (!valid) {
        if (form.value.isExternal) {
          form.value.component = '/views/project/index.vue';
          form.value.name = 'ProjectModel';
          form.value.menuKey = `ProjectModel${getViewKey(form.value.path)}`;
        } else {
          form.value.menuKey = form.value.name;
        }
