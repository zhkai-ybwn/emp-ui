...
const useEchart = (
  getOption,
  options = { isWatch: false, watchData: null }
) => {
  // echarts dom;
  ...
  const renderChart = (rerender = false, ecOption = false) => {
    nextTick(() => {
      if (!echartRef.value)
        throw new Error('[useEcharts Hook] echartRef is not found');
      ...
      if (!getOption)
        throw new Error('[useEcharts Hook] No getOption Function defined');
